...
まず```seq```は```(stream-enumerate-interval 1 20)```の並びに対して、その最初からその項までの和を項とする並びになる。つまり、```(stream-enumerate-interval 1 20)```を \\( \lbrace i_n \rbrace = \lbrace 1, 2, 3, ..., 20 \rbrace \\) と定義した場合、```seq```の各項は、
...
となるため \\( \lbrace s_{n} \rbrace = \lbrace 1, 3, 6, 10, 15, 21, 28, 36, 45, 55, \dots, 210 \rbrace \\) のようになる。

よって、```(stream-ref y 7)``` の結果は \\( \lbrace s_{n} \rbrace \\) の7番目の偶数なので```136```、```(display-stream z)```は \\( \lbrace s_{n} \rbrace \\) の5の倍数の要素なので```10 15 45 55 105 120 190 210```となるはず。
...
メモ化されていない```force```を使う場合では、```seq```を走査する度に```sum```に値が蓄積されてしまうので結果がおかしくなってしまう模様。
