...
ここまで解釈系はRacketを使っているのですが、そのRacketにはデフォルトでstreamsというパッケージがあるので、今回はそれを使います。

* [http://docs.racket-lang.org/reference/streams.html:title]
...
まず```seq```は```(stream-enumerate-interval 1 20)```の並びに対して、その最初からその項までの和を項とする並びになる。つまり、```(stream-enumerate-interval 1 20)```を [tex: \{ i_n \} = \{ 1, 2, 3, ..., 20 \}] と定義した場合、```seq```の各項は、

* [tex: s_{0} = i_{0}]
* [tex: s_{1} = i_{1} + \sum_{k=0}^{0} i_{k}]
* ...
* [tex: s_{n} = i_{n} + \sum_{k=0}^{n-1} i_k]

となるため [tex: \{ s_{n} \} = \{ 1, 3, 6, 10, 15, 21, 28, 36, 45, 55, \dots, 210 \}] のようになる。

よって、```(stream-ref y 7)``` の結果は [tex: \{ s_{n} \}] の7番目の偶数なので```136```、```(display-stream z)```は [tex: \{ s_{n} \}] の5の倍数の要素を出力するので```10 15 45 55 105 120 190 210```となると思う。
