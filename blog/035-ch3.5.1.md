...
まず```seq```は```(stream-enumerate-interval 1 20)```の並びに対して、その最初からその項までの和を項とする並びになる。つまり、```(stream-enumerate-interval 1 20)```を \\( \{ i\_n \} = \{ 1, 2, 3, ..., 20 \} \\) と定義した場合、```seq```の各項は、

* \\( s\_{0} = i\_{0} \\)
* \\( s\_{1} = i\_{1} + \sum\_{k=0}^{0} i\_{k} \\)
* ...
* \\( s\_{n} = i\_{n} + \sum\_{k=0}^{n-1} i\_k \\)

となるため \\( \{ s\_{n} \} = \{ 1, 3, 6, 10, 15, 21, 28, 36, 45, 55, \dots, 210 \} \\) のようになる。

よって、```(stream-ref y 7)``` の結果は \\( \{ s\_{n} \} \\) の7番目の偶数なので```136```、```(display-stream z)```は \\( \{ s\_{n} \} \\) の5の倍数の要素を出力するので```10 15 45 55 105 120 190 210```となると思う。
