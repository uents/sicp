SICP 読書ノート#76 - 5.5 翻訳系(1) (pp.338-xxx)
======================================

いよいよ最後のセクション。

これまでレジスタマシン、超循環評価機(解釈系=インタプリタ)と来て、
ここでは翻訳系(=コンパイラ)について学びます。

翻訳系の概要
--------------------------------------
- ひとことで言うと「環境をenvに保持し、引数リストをarglに集積し、
  適用する手続きをprocに、手続きが戻る場所をcontinueに入れ、
  評価された手続きは答えをvalに入れて戻る」機械語のオブジェクトプログラムを作ること
- インタプリタは式の中でレジスタ命令にぶつかるとすぐに評価するのに対し、
  コンパイラはそれをシーケンスに集積しオブジェクトコードへ変換していく
- これは§4.1.1〜4.1.3の評価機がインタプリタに似ていたのに対し、
  コンパイラは§4.1.7のソースプログラムを解釈して実行手続きを生成する評価機
  に対応している

まさか§4.1がここに至るまでの伏線だったとは。SICPやっぱすごいわ。

5.5.1 翻訳系
--------------------------------------
- §4.1.7の超循環評価機と同じく、コンパイラでも解釈と実行を分離する
...
とりあえず、細かいことは置いといて、
今回も[MITのサンプルコード](ps://mitpress.mit.edu/sicp/code/index.html)から
Racketで動作するコードを作って動かしてみる。
...
評価機の内部をいくら最適化しても、ソースコードを逐次評価して実行する
インタプリタが、コンパイラ以上の最適化は行えないように思う。
