...
導出された式や§4.1.4までの練習問題を解きながら実装していきます。
...
僕の場合は```operands.map```としているので左から。右からとしたい場合はそこを変更すればよいを使えばよい。SICPで処理系をSchemeで実装しているが、同じようなことが言えると思います。
...
いちばん簡単そうなwhileを実装する。問題4.8のnamed letを使うことで
