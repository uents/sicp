...
を公理的定義というようですが、後でも出てくるのでひとまず置いておきます。
...
#### (1) SICPサイトから質問システムのソースコードを取得


#### (2) 質問システムを一部修正

`ch4-query.scm`を一部修正します。
 
- loadに成功するよう`instantiate`と`query-driver-loop`の定義を入れ替え
- `racket/stream`がそのまあ使えるよう`cons-stream`→`stream-cons`
...
これらを`start.scm`のようにひとつにまとめて`(load "start.scm")`としてももちろんよい。
