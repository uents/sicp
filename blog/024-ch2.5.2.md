...
1. apply-genericの中でexp手続きを探すため、```(get 'exp '(complex complex))```
   が実行され```#f```が返る
2. 1の返値は```proc````に格納されるため、```proc```が```#f```となる
   よって、```proc```手続きの実行は行われず、型変換の処理に入る
3. ```type1``` ```type2```とも```'complex```のため、
   ```t1->t2```に```complex->complex```手続きが格納される
4. ところが ```(apply-generic op (t1->t2 a1) a2)``` →
   ```(apply-generic op a1 a2)``` となり、同じ条件で再度apply-genericが実行される
5. 1に戻る
