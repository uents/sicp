SICP 読書ノート#67 - 4.4.4 質問システムの実装(2) (pp.278-290)
======================================
...
- パースしたクエリが`assert!`の場合は、そのassertionかruleをデータベースに追加する
...
この動作は超循環評価器の`eval`と`apply`のそれに似ている。
...
この`(job ?x ?y)`の評価の動きを`trace`を使って追ってみる。
出力されるフレームのストリームは変数`?x`、`?y`の束縛結果の並びである。

ちなみにフレームはp.290まで出てこないけど、変数とそれに束縛される値(シンボル)の並び。
...
1. `fetch-assertions`でインデックス(=先頭のシンボル)が
   `job`で始まるassertionを抽出して取り出す
2. `find-assertions`で1で取り出したassertionsに対しひとつひとつパターンマッチを行い、
   `(? x) (? y)`とマッチするシンボルの対からなるフレームストリーム返す
3. `apply-rules`は今回の場合、空のフレームストリームを返す
4. `simple-query`で3と4のフレームストリームをつなげて返す

といったことを行っている。

図で表すと以下の通り。

![image](https://farm6.staticflickr.com/5705/21076354763_154c035bea_o_d.png)
...
![image](https://farm6.staticflickr.com/5690/21671294196_08a75089f8_o_d.png)
...
![image](https://farm6.staticflickr.com/5791/21074652924_6ab7cfeac9_o_d.png)
...
![image](https://farm1.staticflickr.com/651/21074654214_329494dc1a_o_d.png)
...
![image](https://farm1.staticflickr.com/634/21685889262_cd1f0e0dac_o_d.png)
