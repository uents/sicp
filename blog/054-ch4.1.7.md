...
以下のような実行手続きに変換されるため、並びそのものは実行時に解釈されます。
...
本文のanalyze-sequence手続きでは、さらに踏み込んで以下のような実行手続きに解釈する。
...
どちらも正しく動くけど、Alyssaの例では実行時に並びの解析を行ってしまうので、本文の方がよりよい解析処理と言えるだろう。
